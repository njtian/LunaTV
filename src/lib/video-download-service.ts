  ...
  private async rebuildCacheFromTask(
    ...
    try {
      ...
      if (generatedSeriesKey !== task.series_key) {
        // 如果任务使用的是 source-id 格式（包含 -），而生成的是 douban_ 或 title_ 格式
        // 这是允许的，我们只需确保内容匹配
        const isSourceKey = task.series_key.includes('-');
        if (isSourceKey) {
          console.log(
            `[下载任务 ${task.task_id}] series_key 格式不同（允许）: 任务=${task.series_key}, 生成=${generatedSeriesKey}`
          );
        } else {
          console.error(
